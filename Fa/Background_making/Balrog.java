...
public class Balrog {
    ...
    public Balrog(){
        ...
        for(int i=1;i<=4;i++){//被攻击动画
            img = ImageUtil.getImage("/img/Balrog/Balrog_be_hit/0"+i+".png");
            img_be_hit_r.add(img);
            img = ImageUtil.getImage("/img/Balrog/Balrog_be_hit/01"+i+".png");
            img_be_hit_l.add(img);
        }

        for(int i=1;i<=12;i++){
            // 加载攻击的动画图片
            img = ImageUtil.getImage("/img/Balrog/Balrog_hit/00"+i+".png");
            img_hit_r.add(img);
            img_hit_and_run_r.add(img);
            img = ImageUtil.getImage("/img/Balrog/Balrog_hit/01"+i+".png");
            img_hit_l.add(img);
        ...
        for(int i=1;i<=8;i++){
            // 加载行走的动画图片
            img = ImageUtil.getImage("/img/Balrog/Balrog_run/00"+i+".png");
            img_move_r.add(img);
            img_hit_and_run_r.add(img);
            img = ImageUtil.getImage("/img/Balrog/Balrog_run/01"+i+".png");
            img_move_l.add(img);
            img_hit_and_run_l.add(img);
        }
    }
    ...
    public  void drawSelf(Graphics g){
        ...
        g.drawRect(x+270-10,y+240-30,100,10);
        g.setColor(new Color(205,38,38));
        g.fillRect(x+270-10,y+240-30,100*hp/10,10);
//        g.setColor(Color.black);
//        g.drawRect(x+270,y+240,100,170);
//        g.drawRect(x+370,y+280,120,60);
//        g.drawRect(x+150,y+280,120,60);
//        g.drawLine(x+320,y,x+320,y+700);
//        g.drawLine(x,y+410,x+800,y+410);
...
//            如果没进入，则一直追
    ...
    public boolean hitBy(Player player){
        Mp3Player m2_be_hit=new Mp3Player("/sound/m_be_hit.mp3",false);

        if(player.status==App.STATUS_J ){
            //怪在人的右边，且人向右边攻击
            //判定一次剑气碰撞，剑气(x+350,y+300)width:270;height:60
            //(小怪判定：x+270,y+240) width:100;height=170;
            //y轴的判定点：以怪物和人的脚为参照，▲y<=6*App.Monster_Speed;
            //人物脚的坐标(x+320,y+410)  怪物的脚(x+320,y+510)
            if( player.character ==App.CHARACTER_1 &&
                    (player.x+350+270>=x+270 && player.x+350 <= x+270+100
            ...
            //怪在人的左边，且人向左边攻击
            if( player.character ==App.CHARACTER_1 &&
                    player.x+310-270<=x+270+100 && player.x+310 >= x+270
                    && Math.pow(player.y-y,2) <= Math.pow(App.Monster_Speed*6,2)
                    &&  ( player.dir == App.LEFT
            )){
                if(player.index_j == 6){
            ...
            if( player.character ==App.CHARACTER_2 &&
                    (player.x+360+410>=x+260 && player.x+360 <= x+260+100
                            && Math.pow(player.y-y-40,2) <= Math.pow(App.Balrog_Speed*9,2))
                    &&  (  player.dir == App.RIGHT
            )){
...
//                System.out.println("player.index_j: "+ player.player2_index_j);
        ...
        if(player.status == App.STATUS_O){
            ...
            //人物脚的坐标(x+320,y+410)怪物的脚(x+320,y+510)
            if(Math.pow(player.x - x,2)/(340*340) +
